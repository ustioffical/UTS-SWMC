...
def CalculateVehicleGeoStatusDurationThreshold_Function(queryset):
    ...
    if queryset:
        for i, current_record in enumerate(queryset):
            ...
            ck_vehicle_type = vehicle.vehicle_type
            acc_status = current_record.acc_status.lower() if current_record.acc_status else None  ## Update and below
            ...
            current_vendor_date_time = localtime(current_record.vendor_date_time)  ## remove local_time
            distance = current_record.distance
            current_geom = current_record.geom
            ...
            if acc_status == "on" and speed > 0 and ext_bat_voltage > 0:
                ...
                status = "offline"
            elif acc_status is None or speed is None or ext_bat_voltage is None:  ## ADD
            ...
            if v_code in previous_records:
                ...
                if time_diff.total_seconds() > 60 * 60:  # 60 Minutes
                    ...
                    TrackerRawData.objects.filter(  ## ADD
                        id=tracking_id
                    ).update(gis_geo_status=status.capitalize())

                    if working_data['last_moving_time']:  ## change
                        print("Inside working data last moving time")
                        ...
                        if updated_status == 'Moving' or updated_status == 'Waiting':
                            print("Inside updated status moving or waiting", updated_status)
                            ...
                            for v_code, changes in gis_geo_status_logic_obj.items():
                                tids = changes["tracking_ids"]
                                whs = changes["working_seconds"]

                                # Update gis_geo_status for all IDs (one query)
                                TrackerRawData.objects.filter(id__in=tids).update(gis_geo_status=updated_status)

                                # Bulk update working_hours using Case/When (one query)
                                ...
                                # ----------- CALCULATE DISTANCE CONSECTIVE POINTS  (START)----------
                                if current_geom and prev_record['geom']:
                                    try:
                                        prev_point = prev_record['geom']
                                        ...
                                        ).update(distance=round_distance_km)

                                    except Exception as e:
                                        print(f"Error calculating distance for vehicle {v_code}: {e}")
                                # ----------- CALCULATE DISTANCE CONSECTIVE POINTS  (END)----------
                        else:
                            print("Inside updated status idle or parked", updated_status)
                            for v_code, changes in gis_geo_status_logic_obj.items():
                                TrackerRawData.objects.filter(
                                    id__in=changes["tracking_ids"]
                        ...
                        gis_geo_status_logic_obj[v_code]['working_seconds'].clear()
                        ...
                        print("Inside else working data last moving time - (Parked Override)")
                ...
                # ------------- WORKING AND MOVING DURATION CALCULATION (START)----------
                if working_data['last_moving_time']:
                    ...
                    if prev_status == 'moving' and status == 'moving':
                        ...
                        working_duration_hours = int(working_time_diff.total_seconds() // 3600)
                        working_duration_minutes = int((working_time_diff.total_seconds() % 3600) // 60)
                        working_data['working_seconds'] += float(
                            f"{working_duration_hours}.{working_duration_minutes:02d}")
                        working_hour = working_data['working_seconds']

                        # Update both current and previous records as "Moving"
                        TrackerRawData.objects.filter(
                            id__in=[tracking_id]
                        ).update(working_hours=working_hour)
                    ...
                    if prev_status == 'moving' and status in ['idle', 'parked'] and len(
                            working_data["idle_buffer_between_moving"]) == 0:  ## remove offline
                        if status == 'idle':
                            gis_geo_status_logic_obj[v_code]['tracking_ids'].append(tracking_id)
                            # gis_geo_status_logic_obj[v_code]['tracking_ids'].append(prev_record['id']) ## Moving id of start just ADD
                        ...
                        working_duration_hours = int(working_time_diff // 3600)
                        working_duration_minutes = int((working_time_diff % 3600) // 60)
                        working_data['working_seconds'] += float(
                            f"{working_duration_hours}.{working_duration_minutes:02d}")
                        working_hour = working_data['working_seconds']
                    ...
                    ## Moving (Start) .....(idles) -> idle ...
                        ...
                        gis_geo_status_logic_obj[v_code]['tracking_ids'].append(tracking_id)
                        gis_geo_status_logic_obj[v_code]['working_seconds'].append(working_hour)
                    ...
                    if working_data['next_moving_time']:
                        ...
                        if updated_status == 'Moving' or updated_status == 'Waiting':
                            for v_code, changes in gis_geo_status_logic_obj.items():
                                ...
                                # Bulk update working_hours per Id using Case/When (one query)
                                TrackerRawData.objects.filter(id__in=tids).update(
                                    working_hours=Case(
                                        *[When(id=tid, then=Value(wh)) for tid, wh in zip(tids, whs)],
                                        output_field=FloatField()
                ...
                if status == 'parked':
                    if idle_status_obj[v_code]['total_seconds'] > 0:  # change
                    ...
                    updated_status = TrackerRawData.objects.filter(  # update move below from above
                        id=tracking_id
                    ).update(gis_geo_status='Parked')

                if prev_status == 'parked':  ## chnage
                    vehicle_status_durations[v_code]['parked'] += time_diff
    ...
    working_duration = vehicle_status_durations[v_code]['working']
    working_duration_hours = int(working_duration.total_seconds() // 3600)
    working_duration_minutes = int((working_duration.total_seconds() % 3600) // 60)
    vsgs_working_hour = float(f"{working_duration_hours}.{working_duration_minutes:02d}")

    print(f"Vehicle: {v_code}, Working Duration: {vsgs_working_hour} hours")
    ...
    if ck_vehicle_type:
        ...
        if vehicle_threshold_record:
            ...
            print(
                f"Thresholds - Min Distance: {vtr_min_distance} km, Working Hours: {vtr_working_hour} hours, Ignition Status: {vtr_ignition_status}")
